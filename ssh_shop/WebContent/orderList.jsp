				...
				<tbody>
					<c:forEach items="${order}" var="order">
						<tr>
							...
							&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">
								...
								<c:if test="${order.state==3}">
									<a href="${ pageContext.request.contextPath }/order_updateState.action?oid=${order.oid}">确认收货</a>
								</c:if>
						...
						<%-- <s:iterator var="orderItem" value="order.orderItems"> --%> 
							<tr>
								<td width="60">
								<img width="40" height="45" src="${orderItems.product.productInfor.image}"/>
								</td>
								<td>${orderItems.product.pname}</td>
								<td>${orderItems.product.productInfor.price}</td>
								<td class="quantity" width="60">${orderItems.count}</td>
								<td width="140"><span class="subtotal">￥${orderItems.subtotal}
								</span></td>
							</tr>
						<%-- </s:iterator> --%>
					<%-- </s:iterator> --%> 
